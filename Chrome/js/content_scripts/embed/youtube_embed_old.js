...
$("object").each(function(index) {
    var src = $(this).find("embed").attr("src") || $(this).attr("data");
    var YoutubeId = getYoutubeId(src);
    if(YoutubeId) setting($(this), src, YoutubeId);
});
...
function setting(obj, src, YoutubeId)
{
    // var position = $(this).position();
    // var offset  = $(this).find("embed")[0];

    obj.append(getEmbedButton());

    var button = obj.find("#VideoPins_btn");
